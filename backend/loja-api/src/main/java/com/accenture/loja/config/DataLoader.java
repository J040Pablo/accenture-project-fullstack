...
public class DataLoader implements CommandLineRunner {
    ...
    private void carregarEmpresa() {
        ...
        // empresaService.cadastrar já verifica existência e cria conta da empresa
        try {
            empresaService.cadastrar(dto);
        } catch (Exception ex) {
            // em caso de concorrência ou validação, não interrompe o startup
        }
    }
    ...
    private void carregarProdutos() {
        ...
        criarProdutoSeNaoExistir("NB-DELL-001", "Notebook Dell", "Eletrônicos", new BigDecimal("3500.00"), 10);
        criarProdutoSeNaoExistir("MOUSE-GAMER-001", "Mouse Gamer", "Periféricos", new BigDecimal("150.00"), 30);
        criarProdutoSeNaoExistir("TECLADO-MECA-001", "Teclado Mecânico", "Periféricos", new BigDecimal("300.00"), 15);
        criarProdutoSeNaoExistir("MONITOR-LG-001", "Monitor LG", "Monitores", new BigDecimal("1200.00"), 5);
    }

    private void criarProdutoSeNaoExistir(String sku, String nome, String categoria, BigDecimal preco, Integer estoque) {
        boolean exists = produtoRepository.existsBySku(sku);
        if (exists) return;

        Produto p = new Produto(sku, nome, categoria, preco, estoque);
